...
const services = [
    ...
    },
    {
        ...
        description: 'Promote your business through effective online marketing strategies.',
        icon: '📈',
    },
];
    ...
    return (
        <div>
            {services.map((service, index) => (
                <Card key={index} hoverable style={{ marginBottom: '16px', background: '#ebeeff', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
                    <div style={{ padding: '8px', fontWeight: 'bold' }}>
                        {service.title}
                    </div>
                    <p>{service.description}</p>
                    <div style={{ textAlign: 'center', fontSize: '24px' }}>{service.icon}</div>
                </Card>
            ))}
        </div>
    );
